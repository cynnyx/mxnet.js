...
function start() {
  var modelUrl = "./model/squeezenet-model.json";
  logEvent("Loading " + modelUrl);
   $.getJSON(modelUrl, function(model) {
       ...
       preproc(url, 224, pred.meanimg,  function(nd) {
           ...
           function trainloop(ops, step, nleft, next_goal, finish_callback) {
               if (nleft == 0) {
                 finish_callback(); return;
               }
               var beforeOp = performance.now();
               nleft = pred.partialforward(step);
               var afterOp = performance.now();
               var opName = ops[step].name;
               //No parameters loading
               if (opName.indexOf("weight") == -1 && opName.indexOf("bias") == -1) {
                  logEvent(opName + ": " + (afterOp-beforeOp).toFixed(3));
               }
               progress = (step + 1) / (nleft + step + 1) * 100;
               if (progress >= next_goal || progress == 100) {
                   logProgress(progress);
                   setTimeout(function() {
                       trainloop(ops, step + 1, nleft, next_goal + print_step, finish_callback);
                   }, 1);
               } else {
                   setTimeout(function() {
                       trainloop(ops, step + 1, nleft, next_goal, finish_callback);
                   }, 0);
               }
           }
           trainloop(model.symbol.nodes, 0, 1, 0, function() {
